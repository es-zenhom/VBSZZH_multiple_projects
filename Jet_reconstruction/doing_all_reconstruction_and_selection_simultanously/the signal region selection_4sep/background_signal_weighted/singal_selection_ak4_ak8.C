...
{
    // read file and TTree
    TFile* readf = new TFile ("/home/users/eslam.zenhom/public_html/uf_work/VBSZZH/merged_0.root","READ");
    TTree* Events = (TTree*) readf->Get("Events");
    TFile * f_output =new TFile ("signal.root","recreate");
...
// nevents++;
         ...
         if (ak4_pt_eta_cut_overlap_4d.size()>=2)
         {
             ...
             if (ak8_pt_msoftdrop_cut_4d.size()>=3)
             {
                 ...
                 {
                      hist_pt_leadVBFQuark->Fill(ak4_pt_eta_cut_overlap_4d[position_first_quark_with_high_mqq,weight_of_eachevent].pt());
                      hist_pt_subleadVBFQuark->Fill(ak4_pt_eta_cut_overlap_4d[position_second_quark_with_high_mqq,weight_of_eachevent].pt());
                      hist_eta_leadVBFQuark->Fill(ak4_pt_eta_cut_overlap_4d[position_first_quark_with_high_mqq,weight_of_eachevent].eta());
                      hist_eta_subleadVBFQuark->Fill(ak4_pt_eta_cut_overlap_4d[position_second_quark_with_high_mqq,weight_of_eachevent].eta());
                      hist_phi_leadVBFQuark->Fill(ak4_pt_eta_cut_overlap_4d[position_first_quark_with_high_mqq,weight_of_eachevent].phi());
                      hist_phi_subleadVBFQuark->Fill(ak4_pt_eta_cut_overlap_4d[position_second_quark_with_high_mqq,weight_of_eachevent].phi());

                 }
                 else
                 {
                      hist_pt_leadVBFQuark->Fill(ak4_pt_eta_cut_overlap_4d[position_second_quark_with_high_mqq,weight_of_eachevent].pt());
                      hist_pt_subleadVBFQuark->Fill(ak4_pt_eta_cut_overlap_4d[position_first_quark_with_high_mqq,weight_of_eachevent].pt());
                      hist_eta_leadVBFQuark->Fill(ak4_pt_eta_cut_overlap_4d[position_second_quark_with_high_mqq,weight_of_eachevent].eta());
                      hist_eta_subleadVBFQuark->Fill(ak4_pt_eta_cut_overlap_4d[position_first_quark_with_high_mqq,weight_of_eachevent].eta());
                      hist_phi_leadVBFQuark->Fill(ak4_pt_eta_cut_overlap_4d[position_second_quark_with_high_mqq,weight_of_eachevent].phi());
                      hist_phi_subleadVBFQuark->Fill(ak4_pt_eta_cut_overlap_4d[position_first_quark_with_high_mqq,weight_of_eachevent].phi());

                 }
                 // create mjj (invariant mass of the two vbf jets)
                 hist_mjj_mass_of_twoVBFjets->Fill((ak4_pt_eta_cut_overlap_4d[position_first_quark_with_high_mqq]+ak4_pt_eta_cut_overlap_4d[position_second_quark_with_high_mqq,weight_of_eachevent]).mass());
                 // get the abs eta of the two vbf quarks
                 hist_absDelta_eta_of_twoVBFjets->Fill(TMath::Abs(ak4_pt_eta_cut_overlap_4d[position_first_quark_with_high_mqq].eta()-ak4_pt_eta_cut_overlap_4d[position_second_quark_with_high_mqq,weight_of_eachevent].eta()));
                 ...
                 hist_pt_H_fatjet->Fill(ak8_pt_msoftdrop_cut_4d[position_max_H_score,weight_of_eachevent].pt());
                 hist_eta_H_fatjet->Fill(ak8_pt_msoftdrop_cut_4d[position_max_H_score,weight_of_eachevent].eta());
                 hist_phi_H_fatjet->Fill(ak8_pt_msoftdrop_cut_4d[position_max_H_score,weight_of_eachevent].phi());
...
// the first bracked here is for the if condition that nFatJets large than zero to start redaing the ttres and all that cuz other events doesnt have fat jet in this root file
         ...
         c1->SaveAs("pT of leading VBFquark1.pdf");

         c2->cd();
         hist_pt_subleadVBFQuark->Draw();
         c2->SaveAs("pT of subleading VBFquark1.pdf");

         c3->cd();
         hist_eta_leadVBFQuark->Draw();
         c3->SaveAs("eta of leading VBFquark1.pdf");

         c4->cd();
         hist_eta_subleadVBFQuark->Draw();
         c4->SaveAs("eta of subleading VBFquark1.pdf");

         c5->cd();
         hist_phi_leadVBFQuark->Draw();
         c5->SaveAs("phi of leading VBFquark1.pdf");

         c6->cd();
         hist_phi_subleadVBFQuark->Draw();
         c6->SaveAs("phi of subleading VBFquark1.pdf");

         c7->cd();
         hist_pt_H_fatjet->Draw();
         c7->SaveAs("pT of Higgs1.pdf");

         c8->cd();
         hist_eta_H_fatjet->Draw();
         c8->SaveAs("eta of Higgs1.pdf");

         c9->cd();
         hist_phi_H_fatjet->Draw();
         c9->SaveAs("phi of Higgs1.pdf");

         c10->cd();
         hist_m_H_msoftdrop->Draw();
         c10->SaveAs("msoftdrop of H (mass)1.pdf");

         c11->cd();
         hist_pt_leadboson_fatjet->Draw();
         c11->SaveAs("pT of leadboson1.pdf");

         c12->cd();
         hist_eta_leadboson_fatjet->Draw();
         c12->SaveAs("eta of leadboson1.pdf");

         c13->cd();
         hist_phi_leadboson_fatjet->Draw();
         c13->SaveAs("phi of leadboson1.pdf");

         c14->cd();
         hist_m_leadboson_msoftdrop->Draw();
         c14->SaveAs("msoftdrop of leadboson (mass)1.pdf");

         c15->cd();
         hist_pt_subleadboson_fatjet->Draw();
         c15->SaveAs("pT of subleadboson1.pdf");

         c16->cd();
         hist_eta_subleadboson_fatjet->Draw();
         c16->SaveAs("eta of subleadboson1.pdf");

         c17->cd();
         hist_phi_subleadboson_fatjet->Draw();
         c17->SaveAs("phi of subleadboson1.pdf");

         c18->cd();
         hist_m_subleadboson_msoftdrop->Draw();
         c18->SaveAs("msoftdrop of subleadboson (mass)1.pdf");


         c19->cd();
         hist_mjj_mass_of_twoVBFjets->Draw();
         c19->SaveAs("mjj_mass_of_twoVBFjets1.pdf");

         c20->cd();
         hist_absDelta_eta_of_twoVBFjets->Draw();
         c20->SaveAs("absDelta_eta_mass_of_twoVBFjets1.pdf");

         c21->cd();
         hist_mass_HBosonBoson_system->Draw();
         c21->SaveAs("mass of H_Boson_Boson system1.pdf");
