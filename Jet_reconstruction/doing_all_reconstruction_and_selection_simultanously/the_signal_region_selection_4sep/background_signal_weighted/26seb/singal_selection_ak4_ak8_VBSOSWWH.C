...
void singal_selection_ak4_ak8_VBSOSWWH()
{
    // read file and TTree
    TChain* chain = new TChain("Events");
    TChain* chain1 = new TChain("Runs");
    chain->Add("/ceph/cms/store/user/phchang/VBSVVHSignalSamples_v2/RunIISummer20UL18_VBSOSWWH_incl_C2V_0_Azure_v1/merged/*.root");
    chain1->Add("/ceph/cms/store/user/phchang/VBSVVHSignalSamples_v2/RunIISummer20UL18_VBSOSWWH_incl_C2V_0_Azure_v1/merged/*.root");
    TFile * f_output =new TFile ("sig_VBSOSWWH.root","recreate");
    ...
    float genWeight[333];
    float FatJet_particleNet_WvsQCD[333], FatJet_particleNet_ZvsQCD[333], FatJet_particleNetMD_Xcc[333];

    double genEventSumw[333];
    Long64_t genEventCount[333];
    float Jet_btagDeepFlavB[333];
    ...
    chain->SetBranchAddress("FatJet_particleNet_WvsQCD",&FatJet_particleNet_WvsQCD);
    chain->SetBranchAddress("FatJet_particleNet_ZvsQCD",&FatJet_particleNet_ZvsQCD);
    chain->SetBranchAddress("FatJet_particleNetMD_Xcc",&FatJet_particleNetMD_Xcc);
...
//   chain1->GetEntry(iii);
//
//   sumweights1=sumweights1+genWeight[0];
//
//
// }
...
// cout << "summweights1: " << sumweights<<endl;
...
// nevents++;
          ...
          //   if (Jet_btagDeepFlavB[f1]>0.0490 && Jet_pt[f1]>30)
          //   {
          //     vec_btag_score.push_back(Jet_btagDeepFlavB[f1]);
          //     vec_btag_4d.push_back({Jet_pt[f1],Jet_eta[f1],Jet_phi[f1],Jet_mass[f1]});
          //
          //   }
          // }
         // do the events weights:
         float weight_of_eachevent= (genWeight[0]/sumweights)*0.001747*1000*137;
         ...
         for (int i2 = 0; i2 < nFatJet[0]; i2++)
         {
             if (FatJet_pt[i2]>180 &&FatJet_msoftdrop[i2]>40)
             {
                 ...
                 vec_WvsQCD_score.push_back(FatJet_particleNet_WvsQCD[i2]);
                 vec_ZvsQCD_score.push_back(FatJet_particleNet_ZvsQCD[i2]);
                 vec_Xqq_plus_Xcc_devided_Xqq_plus_Xcc_plus_XCD_score.push_back((FatJet_particleNetMD_Xqq[i2]+FatJet_particleNetMD_Xcc[i2])/(FatJet_particleNetMD_Xqq[i2]+FatJet_particleNetMD_Xcc[i2]+FatJet_particleNetMD_QCD[i2]));
             ...
             if (ak8_pt_msoftdrop_cut_4d.size()>=3)
             {
                 ...
                 // {
                  ...
                  for (int f3 = 0; f3 < vec_btag_score.size(); f3++)
                  {
                        ...
                        double drrr3= sqrt(pow(TMath::Abs(vec_btag_4d[f3].eta()-vec_lead_sublead_bosons_4d[0].eta()),2)+pow(TMath::Abs(ROOT::Math::VectorUtil::DeltaPhi(vec_btag_4d[f3],vec_lead_sublead_bosons_4d[0])),2));
                        double drrr4= sqrt(pow(TMath::Abs(vec_btag_4d[f3].eta()-vec_lead_sublead_bosons_4d[1].eta()),2)+pow(TMath::Abs(ROOT::Math::VectorUtil::DeltaPhi(vec_btag_4d[f3],vec_lead_sublead_bosons_4d[1])),2));
                  ...
                  // cout <<"event: "<< i <<endl;
                  // cout <<"btag_number_inevent:" <<btag_number_inevent<<endl;
                  ...
                  if (vec_lead_sublead_bosons_4d[0].pt()>vec_lead_sublead_bosons_4d[1].pt())
                  {
                      ...
                      hist_Vlead_score->Fill(vec_V_score[0],weight_of_eachevent);
                      hist_Vsublead_score->Fill(vec_V_score[1],weight_of_eachevent);

                      hist_WvsQCD_Vlead->Fill(vec_Vboson_WvsQCD_score[0],weight_of_eachevent);
                      hist_WvsQCD_Vsublead->Fill(vec_Vboson_WvsQCD_score[1],weight_of_eachevent);
                      hist_ZvsQCD_Vlead->Fill(vec_Vboson_ZvsQCD_score[0],weight_of_eachevent);
                      hist_ZvsQCD_Vsublead->Fill(vec_Vboson_ZvsQCD_score[1],weight_of_eachevent);
                      hist_Xqq_plus_Xcc_devided_Xqq_plus_Xcc_plus_QCD_Vlead->Fill(vec_Vboson_Xqq_plus_Xcc_devided_Xqq_plus_Xcc_plus_QCD_score[0],weight_of_eachevent);
                      hist_Xqq_plus_Xcc_devided_Xqq_plus_Xcc_plus_QCD_Vsublead->Fill(vec_Vboson_Xqq_plus_Xcc_devided_Xqq_plus_Xcc_plus_QCD_score[1],weight_of_eachevent);
                  ...
                  else
                  {
                      ...
                      hist_Vlead_score->Fill(vec_V_score[1],weight_of_eachevent);
                      hist_Vsublead_score->Fill(vec_V_score[0],weight_of_eachevent);

                      hist_WvsQCD_Vlead->Fill(vec_Vboson_WvsQCD_score[1],weight_of_eachevent);
                      hist_WvsQCD_Vsublead->Fill(vec_Vboson_WvsQCD_score[0],weight_of_eachevent);
                      hist_ZvsQCD_Vlead->Fill(vec_Vboson_ZvsQCD_score[1],weight_of_eachevent);
                      hist_ZvsQCD_Vsublead->Fill(vec_Vboson_ZvsQCD_score[0],weight_of_eachevent);
                      hist_Xqq_plus_Xcc_devided_Xqq_plus_Xcc_plus_QCD_Vlead->Fill(vec_Vboson_Xqq_plus_Xcc_devided_Xqq_plus_Xcc_plus_QCD_score[1],weight_of_eachevent);
                      hist_Xqq_plus_Xcc_devided_Xqq_plus_Xcc_plus_QCD_Vsublead->Fill(vec_Vboson_Xqq_plus_Xcc_devided_Xqq_plus_Xcc_plus_QCD_score[0],weight_of_eachevent);
                  }

                  // invariant mass of H_Boson_Boson system
                hist_mass_HBosonBoson_system->Fill(ak8_pt_msoftdrop_cut_4d[position_max_H_score].pt()+vec_lead_sublead_bosons_4d[0].pt()+vec_lead_sublead_bosons_4d[1].pt(),weight_of_eachevent);

                hist_pt_scalarsum_VVH_system->Fill((ak8_pt_msoftdrop_cut_4d[position_max_H_score]+vec_lead_sublead_bosons_4d[0]+vec_lead_sublead_bosons_4d[1]).mass(),weight_of_eachevent);
...
// the first bracked here is for the if condition that nFatJets large than zero to start redaing the ttres and all that cuz other events doesnt have fat jet in this root file
         ...
         cout << "cut1: " << cut1 << endl;
         cout << "cut2: " << cut2 << endl;
         cout << "events_count: " << events_count << endl;
         cout << "Nb_lose_nonoverlabbing: " << Nb_lose_nonoverlabbing << endl;
