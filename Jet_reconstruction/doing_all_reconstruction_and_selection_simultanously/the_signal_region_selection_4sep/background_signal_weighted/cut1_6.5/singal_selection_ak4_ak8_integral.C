...
void singal_selection_ak4_ak8_integral()
{
  ...
  for (float i = 0; i < 81; i++)
  {
    if (background->Integral(i,81)>0)
    {
        ...
        g5->AddPoint(i*10/80,sqrt(2 * ((h1->Integral(i,81) + background->Integral(i,81)) * log(1 + h1->Integral(i,81) / background->Integral(i,81)) - h1->Integral(i,81))));
        g6->AddPoint(i*10/80,sqrt(2 * ((h2->Integral(i,81) + background->Integral(i,81)) * log(1 + h2->Integral(i,81) / background->Integral(i,81)) - h2->Integral(i,81))));
        g7->AddPoint(i*10/80,sqrt(2 * ((h3->Integral(i,81) + background->Integral(i,81)) * log(1 + h3->Integral(i,81) / background->Integral(i,81)) - h3->Integral(i,81))));
        g8->AddPoint(i*10/80,sqrt(2 * ((h4->Integral(i,81) + background->Integral(i,81)) * log(1 + h4->Integral(i,81) / background->Integral(i,81)) - h4->Integral(i,81))));
        cout << i << "     "<< (h1->Integral(i,81))/sqrt(background->Integral(i,81))<<endl;
        cout << i << "     "<< sqrt(2 * ((h1->Integral(i,81) + background->Integral(i,81)) * log(1 + h1->Integral(i,81) / background->Integral(i,81)) - h1->Integral(i,81)))<<endl;
  ...
  g5->SetLineColor(1); //g1->SetTitle("VBSWWH");
  g6->SetLineColor(2); //g2->SetTitle("VBSWWH");
  g7->SetLineColor(3); //g3->SetTitle("VBSWWH");
  g8->SetLineColor(4); //g4->SetTitle("VBSWWH");
